...
import { fetchClubConversionRates, upsertClubConversionRates } from '@/services/activityPointService';
import { ClubPointConversion } from '@/types';
import { DEFAULT_ACTIVITY_POINT_CONVERSION } from '@/constants/defaultActivityPointConversion';

export default function PointConversionSettings({ clubId }: { clubId: string }) {
  const [rows, setRows] = useState<ClubPointConversion[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [success, setSuccess] = useState(false);

  useEffect(() => {
    fetchClubConversionRates(clubId)
      .then(data => {
        if (data.length === 0) {
          setRows(DEFAULT_ACTIVITY_POINT_CONVERSION.map(row => ({ ...row, club_id: clubId })));
        } else {
          setRows(data);
        }
      })
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await upsertClubConversionRates(clubId, rows);
      setSuccess(true);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Paper sx={{ p: 3, maxWidth: 600, mx: 'auto', mt: 4 }}>
      <Typography variant="h5" sx={{ mb: 2, color: '#2da58e', fontWeight: 700 }}>Club Activity Point Conversion</Typography>
      <Table size="small">
        <TableHead>
          <TableRow>
